    ...
    def add_pieces(self) -> None:
        for i in range(8):
            ...
            self.white_pieces.append(
                self.setup[i](*coordinates_to_board(i, 7), white=True)
            )
            self.black_pieces.append(Pawn(*coordinates_to_board(i, 1), white=False))
            self.black_pieces.append(
                self.setup[i](*coordinates_to_board(i, 0), white=False)
            )
    ...
    def update(self) -> None:
        if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
            ...
            if isinstance(selected_piece, Piece):
                ...
                if self.selected_piece == board_to_coordinates(
                    ...
                    self.selected_piece = board_to_coordinates(
                        self.current_piece.x, self.current_piece.y
                    )
                    self.from_to_coordinates.append(self.selected_piece)
            else:
                if len(self.from_to_coordinates) != 0:
                    self.from_to_coordinates.append(
                        board_to_coordinates(pyxel.mouse_x, pyxel.mouse_y)
                    )

            if len(self.from_to_coordinates) == 2:
                self.remove_piece(
                    board_to_coordinates(self.current_piece.x, self.current_piece.y)
                )
                self.current_piece.x, self.current_piece.y = coordinates_to_board(
                    self.from_to_coordinates[1][0], self.from_to_coordinates[1][1]
                )
                self.add_piece(
                    self.current_piece,
                    board_to_coordinates(self.current_piece.x, self.current_piece.y),
                )
                if isinstance(self.current_piece, Pawn):
                    self.current_piece.moved = True
                print(
                    notation(
                        self.current_piece.symbol,
                        board_to_coordinates(
                            self.current_piece.x, self.current_piece.y
                        ),
                    )
                )
